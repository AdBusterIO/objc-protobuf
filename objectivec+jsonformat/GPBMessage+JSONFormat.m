...
+ (nullable instancetype)parseFromJSONData:(NSData *)data error:(NSError **)errorPtr {
    return [[self alloc] initWithJSONData:data error:errorPtr];
}

+ (nullable instancetype)parseFromJSONObject:(NSDictionary *)json error:(NSError **)errorPtr {
    return [[self alloc] initWithJSONObject:json error:errorPtr];
}
...
- (nullable instancetype)initWithJSONData:(NSData *)data error:(NSError **)errorPtr {
    ...
    if (![root isKindOfClass:[NSDictionary class]]) {
        if (errorPtr) {
          NSDictionary *userInfo = @{@"Reason": @"Invalid JSON root object"};
          *errorPtr = [NSError errorWithDomain:GPBMessageErrorDomain code:-106 userInfo:userInfo];
        }
        return nil;
    }
    return [self initWithJSONObject:root error:errorPtr];
}

- (nullable instancetype)initWithJSONObject:(NSDictionary *)json error:(NSError **)errorPtr {
    @try {
      GPBMessage *msg = [self init];
      [self.class merge:json message:msg ignoreDefaultValue:YES];
      if (errorPtr) {
        *errorPtr = nil;
      }
      return msg;
...
- (nullable NSData *)JSONData {
    NSDictionary *object = [self.class printMessage:self useTextFormatKey:NO];
    if (object == nil) {
        return nil;
    }
...
+ (void)merge:(nullable NSDictionary *)json message:(GPBMessage *)msg ignoreDefaultValue:(BOOL)ignoreDefVal {
  if (!json || [json isKindOfClass:NSNull.class] || ![json isKindOfClass:NSDictionary.class]) {
    if (!ignoreDefVal) {
      [msg clear];
    }
    return;
...
+ (GPBGenericValue)readValue:(id)val dataType:(GPBDataType)type field:(GPBFieldDescriptor *)field {
  ...
  switch (type) {
    ...
    case GPBDataTypeDouble:
      ...
      break;
    case GPBDataTypeBytes:
      valueToFill.valueData = (__bridge NSData *)(__bridge_retained CFDataRef)[[NSData alloc] initWithBase64EncodedString:val options:0];
      break;
    case GPBDataTypeString:
      valueToFill.valueString = (__bridge NSString *)(__bridge_retained CFStringRef)[val copy];
      break;
    case GPBDataTypeMessage:
    case GPBDataTypeGroup:
    default:
